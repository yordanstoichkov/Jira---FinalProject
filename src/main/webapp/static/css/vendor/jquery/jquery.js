...
(function(global, factory) {
	...
	/* global Symbol */
	// Defining this global in .eslintrc would create a danger of using the
	// global
	...
	(function(window) {
		...
		rcssescape = /([\0-\x1f\x7f]|^-?\d)|^-$|[^\x80-\uFFFF\w-]/g, fcssescape = function(
				ch, asCodePoint) {
			if (asCodePoint) {
		...
		function Sizzle(selector, context, results, seed) {
			var m, i, elem, nid, match, groups, newSelector, newContext = context
				...
				if (documentIsHTML) {
					...
					if (support.qsa && !compilerCache[selector + " "]
						...
						if (nodeType !== 1) {
							...
							// qSA looks outside Element context, which is not
							// what we want
		...
		function siblingCheck(a, b) {
			var cur = b && a, diff = cur && a.nodeType === 1
					&& b.nodeType === 1 && a.sourceIndex - b.sourceIndex;
		...
		function createDisabledPseudo(disabled) {
			...
			// IE: *[disabled]:not(button, input, select, textarea, optgroup,
			// option, menuitem, fieldset)
			// not IE: fieldset[disabled] > legend:nth-of-type(n+2) :can-disable
			return function(elem) {

				// Check form elements and option elements for explicit
				// disabling
				return "label" in elem
						...
						// Check non-disabled form elements for
						// fieldset[disabled] ancestors
						"form" in elem
						&& elem.disabled === false
						&& (
						// Support: IE6-11+
		...
		function addCombinator(matcher, combinator, base) {
			var dir = combinator.dir, skip = combinator.next, key = skip || dir, checkNonElements = base
					&& key === "parentNode", doneName = done++;
		...
		select = Sizzle.select = function(selector, context, results, seed) {
			...
			// Try to minimize operations if there is only one selector in the
			// list and no seed
			...
			// selector above
			(compiled || compile(selector, match))(seed, context,
					!documentIsHTML, results, !context
		...
		support.sortDetached = assert(function(el) {
			...
			return el.compareDocumentPosition(document
	...
	jQuery.fn
			.extend({
				...
				closest : function(selectors, context) {
					var cur, i = 0, l = this.length, matched = [], targets = typeof selectors !== "string"
							&& jQuery(selectors);
				...
				add : function(selector, context) {
					return this.pushStack(jQuery.uniqueSort(jQuery.merge(this
							.get(), jQuery(selector, context))));
				},
	...
	jQuery.Callbacks = function(options) {
		...
		options = typeof options === "string" ? createOptions(options) : jQuery
				.extend({}, options);
	...
	jQuery
			.extend({
				...
				Deferred : function(func) {
					var tuples = [
							...
							[ "reject", "fail",
									...
									"rejected" ] ], state = "pending", promise = {
						state : function() {
							return state;
						},
						always : function() {
								...
								return function() {
									var that = this, args = arguments, mightThrow = function() {
										...
										if (jQuery.isFunction(then)) {
											...
											// Special processors (notify) just
											// wait for resolution
											if (special) {
												then.call(returned, resolve(
														maxDepth, deferred,
														Identity, special),
														resolve(maxDepth,
																deferred,
									...
									// Only normal processors (resolve) catch
									// and reject exceptions
									process = special ? mightThrow
											: function() {
												try {
													mightThrow();
												} catch (e) {
									...
									if (depth) {
										...
										// stack, in case of exception
										// since it's otherwise lost when
										// execution goes async
										if (jQuery.Deferred.getStackHook) {
											process.stackTrace = jQuery.Deferred
													.getStackHook();
										}
				...
				when : function(singleValue) {
					...
					resolveContexts = Array(i), resolveValues = slice
							.call(arguments),
					...
					if (elem) {
						...
						if (data) {
							if (!queue || jQuery.isArray(data)) {
								queue = dataPriv.access(elem, type, jQuery
										.makeArray(data));
							} else {
								queue.push(data);
							}
	...
	function adjustCSS(elem, prop, valueParts, tween) {
		...
		if (valueParts) {
			...
			adjusted = valueParts[1] ? initialInUnit + (valueParts[1] + 1)
	...
	jQuery.event = {
		...
		add : function(elem, types, handler, data, selector) {
			...
			// Evaluate against documentElement in case elem is a non-element
			// node (e.g., document)
			if (selector) {
		...
		dispatch : function(nativeEvent) {
			...
			var i, j, ret, matched, handleObj, handlerQueue, args = new Array(
					arguments.length), handlers = (dataPriv.get(this, "events") || {})[event.type]
	...
	jQuery.Event = function(src, props) {
		...
		if (src && src.type) {
			...
			this.target = (src.target && src.target.nodeType === 3) ? src.target.parentNode
					: src.target;
	...
	cssPrefixes = [ "Webkit", "Moz", "ms" ], emptyStyle = document
	...
	function getWidthOrHeight(elem, name, extra) {
		...
		// value
		var val, valueIsBorderBox = true, styles = getStyles(elem), isBorderBox = jQuery
	...
	jQuery
			.extend({
				...
				css : function(elem, name, extra, styles) {
					...
					// Make numeric if forced or a qualifier was provided and
					// val looks numeric
					if (extra === "" || extra) {
						num = parseFloat(val);
					...
					function(i, name) {
						jQuery.cssHooks[name] = {
							get : function(elem, computed, extra) {
								if (computed) {
									...
									return rdisplayswap.test(jQuery.css(elem,
											...
											// Support: IE <=11 only
											// Running getBoundingClientRect on
											// a disconnected node
											// in IE throws an error.
											(!elem.getClientRects().length || !elem
													.getBoundingClientRect().width) ? swap(
							...
							set : function(elem, value, extra) {
								var matches, styles = extra && getStyles(elem), subtract = extra
	...
	jQuery.cssHooks.marginLeft = addGetHookIf(support.reliableMarginLeft,
			function(elem, computed) {
				if (computed) {
					return (parseFloat(curCSS(elem, "marginLeft")) || elem
	...
	function defaultPrefilter(elem, props, opts) {
		var prop, value, toggle, hooks, oldfire, propTween, restoreDisplay, display, isBox = "width" in props
				|| "height" in props, anim = this, orig = {}, style = elem.style, hidden = elem.nodeType
				&& isHiddenWithinTree(elem), dataShow = dataPriv.get(elem,
				"fxshow");
		...
		if (isBox && elem.nodeType === 1) {
			...
			// Identify a display type, preferring old show/hide data over the
			// CSS cascade
		...
		for (; index < length; index++) {
			...
			if (result) {
				if (jQuery.isFunction(result.stop)) {
					jQuery._queueHooks(animation.elem, animation.opts.queue).stop = jQuery
	...
	jQuery.fn
			.extend({
				...
				finish : function(type) {
					...
					return this
							.each(function() {
								var index, data = dataPriv.get(this), queue = data[type
	...
	jQuery.fx.start = function() {
		if (!timerId) {
			timerId = window.requestAnimationFrame ? window
					.requestAnimationFrame(raf) : window.setInterval(
		...
		propHooks : {
			tabIndex : {
				get : function(elem) {
					...
					return tabindex ? parseInt(tabindex, 10) : rfocusable
							.test(elem.nodeName)
				...
				if (type === "string") {
					...
					if (this.setAttribute) {
						this.setAttribute("class",
								className || value === false ? "" : dataPriv
	...
	jQuery
			.extend({
				valHooks : {
					...
					select : {
						get : function(elem) {
							var value, option, options = elem.options, index = elem.selectedIndex, one = elem.type === "select-one", values = one ? null
									: [], max = one ? index + 1
	...
	if (!support.focusin) {
		jQuery.each({
			...
			jQuery.event.special[fix] = {
				setup : function() {
					var doc = this.ownerDocument || this, attaches = dataPriv
							.access(doc, fix);
					...
					.filter(
							...
							function(i, elem) {
								...
								return val == null ? null
										: jQuery.isArray(val) ? jQuery.map(val,
												function(val) {
													return {
														name : elem.name,
														value : val.replace(
				...
				ajax : function(url, options) {
					...
					jqXHR = {
						...
						setRequestHeader : function(name, value) {
							if (completed == null) {
								name = requestHeadersNames[name.toLowerCase()] = requestHeadersNames[name
										.toLowerCase()]
										|| name;
					...
					if (s.crossDomain == null) {
						...
						// IE throws exception on accessing the href property if
						// url is malformed,
						...
						try {
							...
							// Anchor's host property isn't correctly set when
							// s.url is relative
							...
							// If there is an error parsing the URL, assume it
							// is crossDomain,
	...
	jQuery.expr.pseudos.visible = function(elem) {
		return !!(elem.offsetWidth || elem.offsetHeight || elem
						...
						if (jsonProp || s.dataTypes[0] === "jsonp") {
							...
							jqXHR
									.always(function() {
										...
										// If previous value didn't exist -
										// remove it
										if (overwritten === undefined) {
											jQuery(window).removeProp(
													callbackName);

											// Otherwise restore preexisting
											// value
										} else {
	...
	jQuery.fn.extend({
			...
			if (jQuery.css(elem, "position") === "fixed") {
				...
				parentOffset = {
					top : parentOffset.top
							+ jQuery.css(offsetParent[0], "borderTopWidth",
									true),
		...
		//
		// This logic, however, is not guaranteed and can change at any point in
		// the future
	...
	jQuery.each({
		...
		jQuery.fn[method] = function(val) {
			return access(this, function(elem, method, val) {
				...
				if (win) {
					win.scrollTo(!top ? val : win.pageXOffset, top ? val
							: win.pageYOffset);
